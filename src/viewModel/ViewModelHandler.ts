...
export default class ViewModelHandler {
  ...
  mapDataView() {
    ...
    this.categorical.values
      ...
      .map((m, mi) => {
        ...
        this.viewModel.measureMetadata.push({
          ...
          stroke: DataViewHelper.getMetadataObjectValue<Fill>(
            m,
            "lines",
            "stroke",
            defaultColour
          ...
          selectionId: this.host
            ...
            .createSelectionId(),
          strokeWidth: DataViewHelper.getMetadataObjectValue<number>(
            m,
            "lines",
            "strokeWidth",
            VisualConstants.defaults.lines.strokeWidth
          ),
          lineShape: DataViewHelper.getMetadataObjectValue<string>(
            m,
            "lines",
            "lineShape",
            VisualConstants.defaults.lines.lineShape
          ),
          lineStyle: DataViewHelper.getMetadataObjectValue<string>(
            m,
            "lines",
            "lineStyle",
            VisualConstants.defaults.lines.lineStyle
          ),
          showArea: DataViewHelper.getMetadataObjectValue<boolean>(
            m,
            "lines",
            "showArea",
            VisualConstants.defaults.lines.showArea
          ),
          backgroundTransparency: DataViewHelper.getMetadataObjectValue<number>(
            m,
            "lines",
    ...
    this.smallMultipleColumn.values.map((c: string, ci) => {
      let name =
        c || this.localisationManager.getDisplayName("Visual_Blank_Descriptor");
          ...
          .map((v, vi) => {
            ...
            values.push({
              ...
              selectionId: this.host
                ...
                .withSeries(
                  this.categorical.values,
                  this.categorical.values[vi]
                )
                .createSelectionId(),
              tooltip: value
                ? {
                    header: `${name} - ${valueFormatter.format(
                      category,
      ...
      if (this.settings.smallMultiple.zebraStripe) {
        switch (this.settings.smallMultiple.zebraStripeApply) {
          case "multiple": {
            sm.backgroundColour =
              smi % 2
                ? this.settings.smallMultiple.backgroundColorAlternate ||
                  "transparent"
  ...
  private resolveMasterAxisTickLabel(
    ...
    if (axisSettings.show && axisSettings.showLabels) {
      let length =
          (axis.scale && axis.scale.domain && axis.scale.domain().length) || -1,
        lowest = axis.scale.domain()[0],
        highest = axis.scale.domain()[length - 1],
        lowestFormatted = axis.numberFormat
